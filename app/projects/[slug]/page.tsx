...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><Navbar /><main className="section pt-12"><Link className="text-link" href="/#work">← Back to work</Link><div className="mt-8 grid gap-8 border-t border-white/10 pt-6 lg:grid-cols-[180px_1fr]"><div><p className="eyebrow">{String(project.rank).padStart(2,"0")} / {project.category}</p><p className="status mt-4">{project.status}</p></div><div><h1 className="max-w-5xl text-[clamp(3.2rem,7vw,7.5rem)] font-medium leading-[.9] tracking-[-.065em]">{project.title}</h1><p className="mt-7 max-w-3xl text-xl leading-8 text-[var(--copy)]">{project.subtitle}</p>{project.role ? <p className="mt-4 mono text-[10px] uppercase tracking-[.12em] text-[var(--accent)]">Role · {project.role}</p> : null}<div className="mt-7 flex flex-wrap gap-2">{project.stack.map(item => <span className="chip" key={item}>{item}</span>)}</div><div className="mt-8 flex flex-wrap gap-5">{repositoryIsPublic ? <a className="text-link" href={project.repoUrl} target="_blank" rel="noopener noreferrer">Repository ↗</a> : <span className="mono text-[10px] uppercase tracking-[.1em] text-[var(--muted)]">{project.repoVisibility}</span>}{liveUrlIsSafe ? <a className="text-link" href={project.liveUrl} target="_blank" rel="noopener noreferrer">Live product ↗</a> : null}{project.publication ? publicationUrlIsSafe ? <a className="text-link" href={project.publication.url} target="_blank" rel="noopener noreferrer">{project.publication.label || "Read research paper"} ↗</a> : <span className="button button-disabled" aria-disabled="true">Research paper · {project.publication.status}</span> : null}</div></div></div>{project.visuals?.length ? <section className="mt-10 grid gap-4 lg:grid-cols-2">{project.visuals.map(visual => <figure className="surface overflow-hidden" key={visual.id}><Image src={visual.assetUrl} alt={visual.alt || visual.title} width={1600} height={900} className="aspect-video w-full object-cover" /><figcaption className="p-4 text-sm text-[var(--muted)]">{visual.title}</figcaption></figure>)}</section> : null}<div className="mt-12 grid gap-5 lg:grid-cols-2"><section className="case-panel"><p className="eyebrow">Case study</p><h2 className="mt-4">What this project demonstrates</h2><p className="mt-4">{project.caseStudy}</p></section><section className="case-panel"><p className="eyebrow">Architecture</p><div className="architecture mt-4">{project.architecture}</div></section></div><section className="case-panel mt-4"><p className="eyebrow">Engineering highlights</p><ul className="mt-5 grid gap-3 lg:grid-cols-2">{project.highlights.map(item => <li key={item}>{item}</li>)}</ul></section>{project.outcomes?.length ? <section className="case-panel mt-4"><p className="eyebrow">Outcomes</p><ul className="mt-5 grid gap-3 lg:grid-cols-2">{project.outcomes.map(item => <li key={item}>{item}</li>)}</ul></section> : null}{project.documents?.length ? <section className="case-panel mt-4"><p className="eyebrow">Documents / further reading</p><div className="mt-5 grid gap-3 lg:grid-cols-2">{project.documents.map(document => <article className="surface p-5" key={document.id}><p className="mono text-[10px] uppercase tracking-[.14em] text-[var(--accent)]">PDF</p><h2 className="mt-3">{document.title}</h2><p className="mt-3 text-sm leading-7 text-[var(--copy)]">{document.description}</p><a className="text-link mt-5 inline-flex" href={document.assetUrl} target="_blank" rel="noopener noreferrer">Open document ↗</a></article>)}</div></section> : null}<section className="case-panel mt-4"><p className="eyebrow">Boundaries / limitations</p><p className="mt-4">{project.limitations}</p></section><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd(projectSchema) }} /></main><Footer /></>;
}
