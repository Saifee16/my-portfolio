...
export function NewsletterForm() {
  ...
  return <form onSubmit={submit} className="mt-6"><div className="flex flex-col gap-2 sm:flex-row"><label className="sr-only" htmlFor="newsletter-email">Email address</label><input id="newsletter-email" className="form-input flex-1" type="email" required autoComplete="email" placeholder="you@example.com" value={email} onChange={e=>setEmail(e.target.value)} /><button className="button button-primary" disabled={busy}>{busy ? "Subscribing…" : "Subscribe"}</button></div>{message ? <p className="mt-3 text-sm text-[var(--muted)]" aria-live="polite">{message}</p> : null}</form>;
}
