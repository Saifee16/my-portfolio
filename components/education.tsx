    ...
    <section id="education" className="section">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
        <div className="surface p-6 sm:p-8">
          ...
          {research.map((item, idx) => <div className="mt-7" key={`${item.title}-${idx}`}><h3 className="text-xl font-medium leading-7">{item.title}</h3><p className="mt-2 mono text-[10px] uppercase tracking-[.12em] text-[var(--accent)]">{item.status}</p><p className="mt-2 text-sm text-[var(--muted)]">{item.authors}{item.venue ? ` · ${item.venue}` : ""}{item.year ? ` · ${item.year}` : ""}</p><p className="mt-4 leading-7 text-[var(--copy)]">{item.description}</p><div className="mt-5 flex flex-wrap gap-5">{item.url ? <a className="text-link" href={item.url} target="_blank" rel="noopener noreferrer">Publication ↗</a> : null}{item.doi ? <a className="text-link" href={`https://doi.org/${encodeURIComponent(item.doi)}`} target="_blank" rel="noopener noreferrer">DOI ↗</a> : null}</div></div>)}
        </div>
        <div className="surface p-6 sm:p-8">
          ...
          <Link className="text-link mt-7 inline-flex" href="/certifications">View all certifications →</Link>
        </div>
      </div>
    </section>
  );
