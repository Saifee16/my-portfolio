    ...
    <section id="writing" className="section">
      ...
      <div className="mt-9 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
        <div className="grid gap-5 sm:grid-cols-2">
          {posts.map((post, index) => <article key={post.id} className={`surface overflow-hidden ${index === 0 ? "sm:col-span-2" : ""}`}><Link href={`/blog/${post.slug}`} className="group block">{post.coverImage ? <Image src={post.coverImage} alt="" width={1600} height={900} className="aspect-video w-full object-cover transition duration-300 group-hover:scale-[1.02]" /> : null}<div className="p-6"><p className="eyebrow">{post.category}</p><h3 className={`mt-3 font-medium tracking-[-.04em] group-hover:text-[var(--accent)] ${index === 0 ? "text-3xl sm:text-4xl" : "text-2xl"}`}>{post.title}</h3><p className="mt-3 leading-7 text-[var(--copy)]">{post.excerpt}</p><span className="text-link mt-5 inline-flex">Read note →</span></div></Link></article>)}
