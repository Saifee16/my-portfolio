    ...
    <section id="writing" className="section">
      ...
      <div className="mt-14 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
        <div className="surface p-6 sm:p-8">
          {posts.length ? <div className="space-y-8">{posts.map(post => <article key={post.id} className="border-t border-white/10 pt-6"><p className="eyebrow">{post.category}</p><h3 className="mt-3 text-2xl font-medium tracking-[-.035em]"><a href={`/blog/${post.slug}`} className="hover:text-[var(--accent)]">{post.title}</a></h3><p className="mt-3 leading-7 text-[var(--copy)]">{post.excerpt}</p><div className="mt-5"><a className="text-link" href={`/blog/${post.slug}`}>Read note →</a></div></article>)}</div> : <div className="py-10"><p className="eyebrow">Draft queue ready</p><h3 className="mt-4 text-3xl font-medium tracking-[-.04em]">First engineering notes are being prepared.</h3><p className="mt-4 max-w-xl leading-7 text-[var(--copy)]">Drafts already live in the CMS cover RAG abstention and LLM gateway design. They stay private until published.</p></div>}
